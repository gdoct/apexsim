...
//! Lifts, band columns and colours come from [`track_core::strip_layout`], which the
//! Unreal bake shares.

use bevy::asset::RenderAssetUsages;
use bevy::mesh::{Indices, Mesh, PrimitiveTopology};

use track_core::ats::{Curb, Marking, Side, Surface};
use track_core::strip_layout::{
    surface_height, surface_kind_color, surface_lateral_fractions, surface_lift, CURB_LIFT_M,
    MARKING_LIFT_M,
};
use track_core::terrain::{self, TerrainHeightfield};
use track_core::track_path::{offset_point, CenterlinePath, PathSample};
...
///
/// Returned as one mesh per lateral strip
/// ([`track_core::strip_layout::surface_lateral_fractions`]) so the height profile is
/// sampled across the band's width, not just at its borders.
...
/// The world ground: one mesh over the terrain field's whole grid, seated
/// on [`TerrainHeightfield::ground_height_at`] so it meets the road edge
/// at verge height (a preview at the field's own coarse spacing; the bake
/// refines it beside the road).
pub fn build_ground_mesh(field: &TerrainHeightfield) -> Option<Mesh> {
    ...
    for r in 0..rows {
        for c in 0..cols {
            let (x, y, _) = field.vertex(c, r);
            let z = field.ground_height_at(x, y);
            positions.push(coords::to_bevy(x, y, z).to_array());
...
mod tests {
    use super::*;
    use track_core::ats::{MarkingKind, SurfaceKind};
