...
**`<Track>.uescene.json`** + **`<Track>.uemesh`** (format `apex-ue-scene`, v2) are written by `core/src/ue_export.rs` / `core/src/ue_export_io.rs` and read by the Unreal client at runtime (`FApexTrackSceneReader`), which builds the circuit from them when it is raced; there are no cooked track levels (`docs/RUNTIME_CONTENT_LOADING.md`). The editor-only `ApexTrackImport` commandlet reads them too, to import a circuit as a level for inspection. They are *generated* files: they land in `content/tracks/export/` (gitignored — Zandvoort alone is a 1.2 MB manifest and a 7.6 MB blob), never beside the source content.
