...
# What the game shows instead of the real (trademarked) name above; the
# YAML keeps both, `name` and `display_name`.
declare -A DISPLAY_NAMES=(
    ["Austin"]="Circuit of the Armadillos"
    ["BrandsHatch"]="Brands Scratch"
    ["Budapest"]="Hungoverring"
    ["Catalunya"]="Circuit de Barcelunatic"
    ["Hockenheim"]="Hockeyheimring"
    ["IMS"]="Endianapolis Motor Speedway"
    ["Melbourne"]="Albert Parkour Circuit"
    ["MexicoCity"]="Autódromo Hermanos Rodri-Queso"
    ["Montreal"]="Circuit Chilly Villeneuve"
    ["Monza"]="Autodromo Monzarella"
    ["MoscowRaceway"]="Moscow Mule Raceway"
    ["Norisring"]="Snorisring"
    ["Nuerburgring"]="Nürburgerring"
    ["Oschersleben"]="Motorsport Arena Oskarsleben"
    ["Sakhir"]="Bahrainless International Circuit"
    ["SaoPaulo"]="Autódromo Interlaggos"
    ["Sepang"]="Shebang International Circuit"
    ["Shanghai"]="Shanghaied International Circuit"
    ["Silverstone"]="Shiverstone Circuit"
    ["Sochi"]="Gnocchi Autodrom"
    ["Spa"]="Spa-Frankenchamps"
    ["Spielberg"]="Red Pull Ring"
    ["Suzuka"]="Sudoku Circuit"
    ["YasMarina"]="Yas Marinara Circuit"
    ["Zandvoort"]="Zandervoort"
)
...
for track_csv in "$TRACKS_DIR"/*.csv; do
    ...
    parody="${DISPLAY_NAMES[$track_name]}"
    if [ -n "$parody" ]; then
        cmd="$cmd --display-name \"$parody\""
    fi
