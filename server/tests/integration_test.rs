...
impl TestClient {
    ...
    async fn authenticate(
        &mut self,
    ) -> Result<(PlayerId, ServerMessage), Box<dyn std::error::Error>> {
    ...
    async fn select_car(&mut self, car_id: CarConfigId) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let ServerMessage::LobbyState(lobby) = &response {
            ...
            {
                println!(
                    "DEBUG: Player car selection in lobby: {:?}",
                    player.selected_car
                );
            }
        }
            ...
            let response = match timeout(Duration::from_secs(5), self.receive_tcp_message()).await {
                ...
                Err(_) => {
                    if start.elapsed() > Duration::from_secs(10) {
                        return Err(
                            "Timeout waiting for session creation response from server".into()
                        );
                    }
...
async fn test_multiplayer_race_session() {
    ...
    let result = timeout(TEST_TIMEOUT, async {
        ...
        let mut client1 = TestClient::connect("Player1", server.tcp_addr).await?;
        let mut client2 = TestClient::connect("Player2", server.tcp_addr).await?;
        let mut client3 = TestClient::connect("Player3", server.tcp_addr).await?;
        let mut client4 = TestClient::connect("Player4", server.tcp_addr).await?;
        ...
        assert!(
            ...
            "Clients should receive synchronized ticks"
        );

        println!(
            "✓ All clients successfully connected, joined session, and received synchronized data"
        );
...
/// Run: cargo test --test integration_test test_telemetry_broadcast -- --nocapture
#[tokio::test]
async fn test_telemetry_broadcast() {
    ...
    let result = timeout(Duration::from_secs(60), async {
        ...
        let mut client1 = TestClient::connect("BroadcastTest1", server.tcp_addr).await?;
        let mut client2 = TestClient::connect("BroadcastTest2", server.tcp_addr).await?;
        ...
        while start.elapsed() < test_duration {
            ...
            if let Some((server_tick, car_count)) = client1.receive_telemetry().await? {
                if client1_telemetry_count == 0 {
                    println!(
                        "  🎯 Client 1 received FIRST telemetry: tick={}, cars={}",
                        server_tick, car_count
                    );
                }
            ...
            if let Some((server_tick, car_count)) = client2.receive_telemetry().await? {
                if client2_telemetry_count == 0 {
                    println!(
                        "  🎯 Client 2 received FIRST telemetry: tick={}, cars={}",
                        server_tick, car_count
                    );
                }
        ...
        println!(
            "  Client 1 received {} telemetry packets",
            client1_telemetry_count
        );
        println!(
            "  Client 2 received {} telemetry packets",
            client2_telemetry_count
        );
    ...
    match result {
        ...
        Err(_) => {
            panic!("Test timed out - server may not be responding");
        }
    }
}
...
async fn test_sandbox_session_workflow() {
    ...
    let result = timeout(TEST_TIMEOUT, async {
        ...
        let mut client = TestClient::connect("SandboxPlayer", server.tcp_addr).await?;
        ...
        let session_id = client
            .create_session(track_id, 1, SessionKind::Sandbox)
            .await?;
        ...
        assert!(
            telemetry_received,
            "Did not receive telemetry (race may not have started)"
        );
