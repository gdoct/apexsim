...
VARIANTS = {
    "posh": dict(
        ...
        wheels=((-1.35, 0.265), (1.40, 0.275)), arch_r=0.42, glass_y=(-0.95, 1.15), screen_y=(-0.95, -0.18), rear_glass_y=(0.55, 1.15),
        ...
        keys=[
            ...
            (-1.90, [(0, .10), (.85, .10), (.95, .35), (.97, .62), (.80, .76), (.60, .80), (.40, .82), (.18, .84), (0, .85)]),
            (-1.35, [(0, .08), (.90, .08), (.99, .40), (1.0, .68), (.82, .80), (.62, .84), (.42, .86), (.18, .88), (0, .88)]),
            (-0.80, [(0, .08), (.90, .08), (.98, .40), (.98, .66), (.84, .78), (.68, .90), (.50, 1.05), (.28, 1.18), (0, 1.22)]),
    ...
    "limbotiti": dict(
        ...
        keys=[
            ...
            (-1.35, [(0, .08), (.92, .08), (1.00, .36), (1.01, .64), (.84, .76), (.64, .80), (.44, .82), (.20, .84), (0, .85)]),
            (-0.85, [(0, .08), (.92, .08), (1.00, .36), (1.00, .66), (.88, .80), (.76, .84), (.60, 1.00), (.30, 1.08), (0, 1.09)]),
    ...
    "murcetes": dict(
        ...
        wheels=((-1.45, 0.265), (1.35, 0.275)), arch_r=0.42, glass_y=(-0.45, 1.20), screen_y=(-0.45, 0.30), rear_glass_y=(0.85, 1.20),
        ...
        keys=[
            ...
            (-1.45, [(0, .08), (.92, .08), (1.00, .40), (1.01, .70), (.84, .84), (.64, .88), (.44, .90), (.18, .92), (0, .93)]),
            (-0.80, [(0, .08), (.92, .08), (1.00, .40), (1.00, .70), (.86, .82), (.66, .88), (.46, .92), (.20, .94), (0, .95)]),
            (-0.20, [(0, .08), (.92, .08), (1.00, .40), (1.00, .70), (.86, .82), (.70, .92), (.52, 1.08), (.28, 1.20), (0, 1.24)]),
...
sp, sg = b.slot(paint), b.slot(glass); WIN0, WIN1 = 5 * SAMP + 1, 7 * SAMP - 1   # window band: window line up to the roof edge
...
p.box(interior, (-0.80, -0.55, 0.60), (0.80, -0.20, 0.85))
p.box(display, (DX - 0.12, -0.21, 0.66), (DX + 0.12, -0.20, 0.78))
p.bar(metal, (DX, -0.25, 0.70), (DX, 0.05, 0.74), 0.02)
wr = [Vector((DX + 0.16 * math.cos(2 * math.pi * i / 24), 0.05, 0.74 + 0.16 * math.sin(2 * math.pi * i / 24))) for i in range(24)]
for i in range(24): p.bar(alc, wr[i], wr[(i + 1) % 24], 0.018, segs=6)
p.box(alc, (DX - 0.15, 0.04, 0.72), (DX + 0.15, 0.065, 0.76)); p.box(alc, (DX - 0.03, 0.04, 0.58), (DX + 0.03, 0.065, 0.74))
...
s = p.slot(logo)
for sx in (-1, 1):
    x = sx * 1.045; y0, y1, z0, z1 = -0.55, 0.65, 0.40, 0.80
